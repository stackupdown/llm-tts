...
def main(args, config):
    ...
    for file in files:
        ...
        if os.path.exists(root_path + f"/{test_audio}/audio/" +f"{file}/"):
            r = glob.glob(root_path + f"/{test_audio}/audio/" +f"{file}/*")
            for j in r:
                os.remove(j)
        texts = []
        prompts = []
        ...
        for i, (speaker, prompt, text, content) in enumerate(tqdm(zip(speakers, prompts, texts, contents))):
            ...
            with torch.no_grad():
                ...
                if not os.path.exists(root_path + f"/{test_audio}/audio/" +f"{file}/"):
                    os.makedirs(root_path + f"/{test_audio}/audio/" +f"{file}/", exist_ok=True)
                sf.write(file=root_path + f"/{test_audio}/audio/" +f"{file}/{i+1}.wav", data=audio, samplerate=config.sampling_rate) #h.sampling_rate
...
if __name__ == '__main__':
    ...
    p.add_argument('--sub_dir', type=str, default='audio', help="the subdir of the whole output dir, e.g. audio of prompts/audio")
